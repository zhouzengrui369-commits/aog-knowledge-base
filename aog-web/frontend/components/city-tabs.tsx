  ...
  return (
    <div className="prose-city max-w-none">
      ...
      <table>
        <tbody>
          <tr>
            <th className="bg-ink-50">机场名称</th>
      ...
      <p className="text-xs text-ink-500">提示：库存为该航站自营库存；× 表示无库存，需协议求援。</p>
    </div>
  );
}
...
function ContactsPane({ city }: { city: City }) {
  ...
  if (contacts.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-ink-100 bg-ink-50 p-6 text-center text-sm text-ink-500">
  ...
  return (
    <div>
      <h2 className="mb-3 text-lg font-semibold text-ink-900">当地及周边资源</h2>
      <p className="mb-4 text-sm text-ink-500">
        当自营航材不足时，可按以下联系方式求援 / 中介 / 互援。
      </p>
...
function PartsPane({ city }: { city: City }) {
  ...
  return (
    <div>
      <h2 className="mb-3 text-lg font-semibold text-ink-900">航材备件清单</h2>
          ...
          <tbody className="divide-y divide-ink-100">
            {parts.map((p, i) => (
              <tr key={i}>
                ...
                <td className="px-3 py-2 font-mono text-xs text-ink-700">{p.pn}</td>
...
function LogisticsPane({ city }: { city: City }) {
  ...
  return (
    <div>
      <h2 className="mb-3 text-lg font-semibold text-ink-900">物流方案</h2>
      <p className="mb-4 text-sm text-ink-500">
        从其他基地 / 国际件库到本场的主要运输方式与时效参考。
      </p>
        ...
        {logs.map((l, i) => (
          <div key={i} className="rounded-lg border border-ink-100 bg-white p-4">
            <div className="mb-1 flex items-center gap-2">
              ...
              <div className="text-sm font-semibold text-ink-900">{l.type}</div>
            </div>
...
function WarehousePane({ city }: { city: City }) {
  ...
  return (
    <div>
      <h2 className="mb-3 text-lg font-semibold text-ink-900">仓储单位</h2>
      <div className="rounded-xl border border-ink-100 bg-ink-50 p-5">
        ...
        {w.address && <div className="mt-1 text-sm text-ink-700">地址：{w.address}</div>}
        {w.phone && (
          <div className="mt-1 text-sm text-ink-700">
            电话：
            <a href={`tel:${w.phone}`} className="text-primary hover:underline">
              {w.phone}
            </a>
          </div>
        )}
        {w.owners && <div className="mt-1 text-sm text-ink-700">负责人：{w.owners}</div>}
      </div>
    </div>
  );
}
