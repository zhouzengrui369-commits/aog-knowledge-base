...
export function CityDetailClient({ code }: { code: string }) {
  ...
  return (
    <>
      ...
      <div className={`mt-4 border-y ${verified ? "border-green-200 bg-green-50" : "border-amber-200 bg-amber-50"}`}><div className="mx-auto max-w-7xl px-4 py-3 text-sm sm:px-6 lg:px-8"><div className="flex flex-wrap items-center gap-2"><strong>{verified ? "已核验数据" : "数据未审核，禁止用于实际处置"}</strong><span className="rounded-full bg-white/70 px-2 py-0.5 text-xs">{reviewStatus}</span>{city.trust?.confidence != null && <span className="text-xs">置信度 {(city.trust.confidence * 100).toFixed(0)}%</span>}{!verified && <Link href={`/review/city/${encodeURIComponent(city.code)}`} className="ml-auto rounded-md border border-amber-300 bg-white px-3 py-1.5 text-xs font-semibold text-amber-900 hover:border-primary hover:text-primary">进入知识审核（只读）</Link>}</div><div className="mt-1 flex flex-wrap gap-x-4 gap-y-1 text-xs text-ink-600">{city.trust?.source_document && <span>来源：{city.trust.source_document}</span>}{city.trust?.reviewed_by && <span>审核：{city.trust.reviewed_by}</span>}{city.trust?.updated_at && <span>更新：{fmtDate(city.trust.updated_at)}</span>}</div>{!verified && <p className="mt-2 text-xs text-amber-800">运营页面继续隐藏未核验数据；登录后的审核页面可查看脱敏候选内容，用于人工核对。</p>}</div></div>

      <header className="mx-auto max-w-7xl px-4 pt-6 sm:px-6 lg:px-8"><div className="rounded-2xl border border-ink-100 bg-white p-6 shadow-soft"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${status.cls}`}>{status.text}</span><span className="rounded-full bg-ink-50 px-2.5 py-0.5 text-xs text-ink-600">城市：{city.name}</span><span className="rounded-full bg-ink-50 px-2.5 py-0.5 text-xs text-ink-600">机场：{city.airport || "待核"}</span></div><h1 className="mt-3 text-3xl font-bold text-ink-900">{city.name} <span className="font-mono text-xl text-ink-400">{city.iata || "—"}</span></h1><p className="mt-2 text-sm text-ink-500">{city.region} · 城市与机场名称分开管理，三字代码以审核数据为准。</p><div className="mt-6 grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-ink-100 bg-ink-100 sm:grid-cols-4"><Stat icon={<Eye className="h-4 w-4" />} label="访问次数" value={viewCount > 1 ? String(viewCount) : "首次访问"} /><Stat icon={<Plane className="h-4 w-4" />} label="已核验机型" value={verified ? `${city.fleet?.length || 0} 种` : "需审核"} /><Stat icon={<Package className="h-4 w-4" />} label="已核验备件" value={verified ? `${city.parts?.length || 0} 项` : "需审核"} /><Stat icon={<Clock className="h-4 w-4" />} label="数据状态" value={reviewStatus} /></div></div></header>

      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8"><div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr,300px]"><CityTabs city={city} /><aside className="space-y-4"><div className="rounded-lg border border-ink-100 bg-white p-5"><div className="text-xs font-semibold uppercase tracking-wider text-ink-500">处置目标与责任边界</div><div className="mt-3 text-lg font-semibold text-ink-900">30 分钟内完成首次信息核对</div><p className="mt-2 text-xs leading-5 text-ink-500">执行责任方：当班航材 AOG 工程师。该时间是内部处置目标，不代表航司、机场、供应商或本平台对外 SLA；实际承诺以书面协议为准。</p></div><div className="rounded-lg border border-ink-100 bg-white p-5 text-xs leading-5 text-ink-500"><strong className="text-ink-700">相关航站算法</strong><p className="mt-1">优先推荐同地区航站，不足时按现行列表补足。该标签不代表互援协议或库存可用。</p></div></aside></div>
